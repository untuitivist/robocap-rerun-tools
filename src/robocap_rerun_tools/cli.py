...
def command_modelscope_upload(args: argparse.Namespace) -> int:
    ...
    print(f"Upload date: {result.batch_id or 'not assigned'}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    modelscope_stage_parser.add_argument(
        ...
        required=True,
        help=(
            "Action directory name; one or more letters plus one or more digits is the "
            "compact convention, and custom names are accepted."
        ),
    )
    ...
    modelscope_upload_parser = sub.add_parser(
        "modelscope-upload",
        help="Upload all prepared sessions under one YYYYMMDD/name/session_id date path.",
    )
