...
is the Dataset Card. `raw_calibration/<device_id>/` is the required calibration collection maintained
by a separate workflow, while `EgoMotionActions/` is generated by session staging.
...
`manifest.json` and the inspection HTML. Raw calibration stays outside every action session; the
session manifest and `metadata.jsonl` contain explicit main, left, and right device IDs.
...
由 Session 准备流程生成的数据目录。根级 `raw_calibration/<device_id>/` 是必需的标定数据区，由
独立标定流程维护。
...
原始标定数据位于每个动作 session 之外，Session 的 manifest 与 `metadata.jsonl` 只记录明确的
main、left、right device ID，用于查找根级标定目录。
...
LANGUAGE_PACKS = {
    "English": {
        ...
        "modelscope_help": (
            ...
            "- `raw_calibration/<device_id>/`: required; maintained by the calibration workflow.\n"
            ...
            "- Action sessions reference root calibration only by explicit main/left/right IDs.\n\n"
    ...
    "中文": {
        ...
        "modelscope_help": (
            ...
            "- `raw_calibration/<device_id>/`：必需，由独立标定流程维护。\n"
            ...
            "- 动作 session 只用明确的 main/left/right device ID 引用根级标定。\n\n"
