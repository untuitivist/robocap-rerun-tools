...
普通上传、批量上传及错误库上传都在返回成功前，对本次暂存文件核对远端路径、大小和
SHA-256，并复核本次 Session 的元数据。上传前检查 manifest 中的文件是否存在、大小是否
吻合，按块计算哈希，不将整段视频加载到内存。远端没有提供 SHA-256 时，强制下载该文件
计算哈希，因此可能增加下载流量和本地缓存占用。

缺失或内容不一致的文件单独关闭上传缓存补传，最多补传三次，每次重新核验。仍有问题则
报失败，批量任务继续下一批，不把未通过的批次计入成功。远端查询失败会明确报告无法校验，
不会盲目覆盖整批数据。元数据补传前重新读取远端索引，保留其他 Session。

Web/CMD 输出逐文件校验与补传进度；每批暂存根目录保存 `upload_verification.json`，包含
本次文件大小、哈希、各轮问题和最终状态。此校验确认上传内容与本地暂存文件一致，
不代表原始视频一定能解码、动捕格式一定兼容下游，或未被选择上传的文件已补齐。
原始文件本身损坏需要换用有效源文件，不能通过重复上传修复。视频不会因此被压缩。
...
目录中的唯一 `P<一位或多位数字>` 归类；无此编号时读取其他紧凑动作 ID。动作前缀可含一个或多个
英文字母，数字位数不限且前导零会保留。当前目录包含 `P、D、SM、LM、DR、HH、EC、LH` 系列。
每个 Segment 只选择一个 Robocap 参考视频计算时长，不会把多摄像头重复
相加。默认会按选择的 8/4 倍比例串行补做缺失检查，再输出每个动作的未检查时长、差帧时长、无误
时长、总时长、`{Session: Session 时长}` 和逐 Session 异常列表。三个时长类别互斥，且
...
数据集总览还会汇总 Session 数量：全部、无误、有错误和未检查。其中“有错误”包含
未检查，因此 `无误 Session + 有错误 Session = 全部 Session`；“未检查”作为诊断子集单独列出。
错误类型统计分别计算未检查、mocap 多帧/少帧和第三人称多帧/少帧涉及的 Session 数，同时保留
逐 Session 异常明细用于定位。

未检查不会只显示一个笼统标签，而是按 Session 当前文件状态区分为：没有直属 `mocap*` 文件夹、
`mocap*` 中递归找不到 TRC/BVH/CSV、已有上述动捕文件但没有有效检查报告。目录发现与其他功能
共用同一套大小写不敏感的 `mocap*` 规则。

统计页的 Mocap 补回工具可指定另一个候选根目录。它递归发现其中名称以 `mocap` 开头且包含
TRC/CSV/BVH 的文件夹，将文件夹创建时间与缺失 Session 名称中的 `YYYYMMDD_HHMMSS` 做全局一对一
最近匹配。Session 名称时间按 UTC 解释，候选文件夹创建时间按东八区显示，再按带时区的绝对时刻比较。
缺少轨迹文件和检查后确认帧数不对齐的 Session 都会参与。必须先用“预览 Mocap 匹配”
检查时间差以及将被删除的目录，再勾选替换确认并执行复制。候选与现有 `mocap*` 目录名称一致时跳过；
名称不一致时事务式删除旧目录并完整复制候选，复制失败会恢复旧目录。一个 Session 中存在多个
`mocap*` 时同样参与补回：预览列出全部旧目录，确认后全部删除并仅复制最近匹配的一个候选。无法解析
任何实际新增或替换 Mocap 的操作也会删除该 Session `_artifacts` 下已经失效的时间戳检查 HTML，便于
统计页补做报告；复制期间报告会先备份，失败时与旧 Mocap 一起恢复。无法解析时间、没有匹配项等情况进入人工检查列表。候选源目录不得与数据集目录重叠，源文件不会移动或删除。
输出末尾固定列出候选中不含 TRC/CSV/BVH 的 `mocap*` 文件夹。预览和复制操作会持续输出 Session
扫描、候选根目录遍历、匹配构建和逐 Session 替换进度；日志长度有上限，避免大目录拖高 Web 内存。

数据集总览中的时长也使用同样的全部、无误、有错误、未检查汇总，其中
`无误时长 + 有错误时长 = 全部时长`，未检查时长是有错误时长的子集。具体错误类型时长按受影响
Segment 的 Robocap 参考视频时长累计；同一 Segment 同时出现多种错误时会分别计入，因此各错误
类型时长不能相加作为有错误时长。
这些分类汇总位于表格上方；动作明细表不显示分类汇总列或 Session 数列。
...
自动建议动作基元；找不到时读取其他紧凑动作 ID。动作前缀可包含一个或多个英文字母，数字位数不限
并保留前导零。例如 `mocap-SM03-S7-user-5p` 会建议 `SM03`。这只是默认值；下拉框允许任意安全的
单级目录名，手动输入具有最终
