...
- Build a labeled frame-comparison JPEG with selected Robocap-video, third-person-video, and
  BVH/TRC/CSV/XRS/C3D columns over an inclusive Robocap frame range.
...
The `Frame Comparison` tab separates sources into Robocap reference videos, third-person videos, and
Mocap 3D files (`.bvh`, `.trc`, `.csv`, `.xrs`, `.c3d`). Start and end indexes are 0-based inclusive
Robocap frames. Each selected source occupies one column and each requested Robocap frame occupies
one row. Video cells preserve aspect ratio with black padding; Mocap cells use one fixed camera and
scale for the full column, with deep-blue points and red connections. Every cell is `960 x 540` by
default and labels both the Robocap frame and mapped source frame. Robocap frame `N` selects Mocap
frame `round(N*ratio) + round(mocap_offset*ratio)` and third-person frame
`N + third_person_offset`. Both offsets are independent and signed. The full image is written to
`<session>/_artifacts/frame_comparison/`; the Web page shows live cell-level progress, a downloadable
file, and a button that opens it in the operating system's default image application. The tall image
is not rendered inline. Its RGB canvas is disk-backed to keep Python memory bounded.
...
Robocap is the reference for both frame comparison and frame-aligned RRD export. They share this
mapping:
...
`--offset` controls Mocap only and is measured in Robocap frames. Positive values select later Mocap
frames for the same Robocap frame; negative values select earlier Mocap frames. The exporter converts
it to `round(offset * ratio)` Mocap source frames. At ratio 8, `--offset 5` becomes Mocap offset `40`,
while `--offset -5` places Mocap frame 0 at Robocap frame 5. `--third-person-offset` is independent:
positive `T` selects third-person frame `N+T` for Robocap frame `N`, while negative `T` selects an
earlier third-person frame. It is not multiplied by the Mocap ratio.

Frame-aligned RRD files use `frame` as the primary timeline, expressed at the GT/NOKOV frame
rate. Every 30 FPS Robocap source frame `N` is logged at `frame = round(N*ratio)`. Mocap source frame
`K` is logged at `frame = K - round(mocap_offset*ratio)`. Third-person source frame `M` is logged at
`frame = round((M-third_person_offset)*ratio)`. Consequently all three cells selected from one
Robocap frame land on the same primary frame value, while changing either offset cannot move the
other source. MP4 PTS jitter cannot change these primary frame positions. Sensors are mapped onto the
same integer frame axis from the reference video's timestamps. Media timing remains on the secondary
`capture_time` timeline; raw videos retain their source PTS, while proxies use normalized 30 FPS PTS.
...
ratio (`r`), Mocap offset (`mo`), third-person offset (`to`), reference video (`ref`), frame range (`f`, or `fall`),
